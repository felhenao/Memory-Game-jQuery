$(document).ready(function() {
	let divs = [];	// to store the random boxes that will be shown
	let clickedDivs = [];// to store the boxes that the user clicked on
	let divsToFlash = 3;// the amount of boxes that are going to be flashed
	let matchingDivs = []; // to store the clicked divs that match with the flashed divs
	let failedAttempts = 0;// this keeps track and decides when the user looses
	let currentLevel = 1;// if the user meets the criteria to pass to next level then currentLevel variable will be increased by one
	let $r1c1 = $('.r1c1');
	let $r1c2 = $('.r1c2');
	let $r1c3 = $('.r1c3');
	let $r1c4 = $('.r1c4');
	...
	let $r4c3 = $('.r4c3');
	let $r4c4 = $('.r4c4');
	let cubes = [$r1c1, $r1c2, $r1c3, $r1c4, $r2c1, $r2c2, $r2c3, $r2c4, $r3c1, $r3c2, $r3c3, $r3c4, $r4c1, $r4c2, $r4c3, $r4c4];// note: r1c1 === row one column  one
	...
	$('#playB').on('click', flashDivs);// play button. When the user clicks on the play button, random boxes change color
	$('button.letsStart').on('click', greet);// let's start button from the landing page
	...
	function flashDivs() {
		randomNums();// to run the randomNums() function with divsToFlashas parameter to indicate the amount of boxes to flash according to the current level
  ...
  // this makes everything on the screen disapear but the container box so the user is not distracted by anything else on the screen and the container rotates 90 degrees
	...
	function rightOrWrong() {
		for (let i = 0; i < divs.length; i++) {// iterate through the flashed divs to look for matches between the clicked and the flashed divs
	...
	function ifWrong() {
		...
		if (failedAttempts === 3){
			...
			setTimeout(function () {
				$('.dialog').text(`Level ${currentLevel}`);
			}, 2000);// this displays the level 1  so the user has the option to play again
		}
